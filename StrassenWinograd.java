public class StrassenWinograd {
    ...
    public static long[][] multiply(int[][] a, int[][] b) {
        return strassenWinograd(toLong(a), toLong(b));
    }

    private static long[][] strassenWinograd(long[][] a, long[][] b) {
        int n = a.length;
        long[][] c = new long[n][n];

        // Caso base facil de ubicar: cuando n es pequeno usa multiplicacion clasica.
        if (n <= UMBRAL) {
            for (int i = 0; i < n; i++) {
                for (int j = 0; j < n; j++) {
                    long suma = 0;
        ...
        long[][] m1 = strassenWinograd(sumar(a11, a22), sumar(b11, b22));
        long[][] m2 = strassenWinograd(sumar(a21, a22), b11);
        long[][] m3 = strassenWinograd(a11, restar(b12, b22));
        long[][] m4 = strassenWinograd(a22, restar(b21, b11));
        long[][] m5 = strassenWinograd(sumar(a11, a12), b22);
        long[][] m6 = strassenWinograd(restar(a21, a11), sumar(b11, b12));
        long[][] m7 = strassenWinograd(restar(a12, a22), sumar(b21, b22));
